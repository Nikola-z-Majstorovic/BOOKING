    ...
    public class CommentsController : BaseApiController, IBaseActions
    {
        ...
        public HttpResponseMessage Get(Guid objId)
        {
            if (ModelState.IsValid)
            {
                ...
                MasterBService.Comment[] commentListFromMB = client.GetCommentsForOwnerId(GetUserId());
    
                List<Comment> freshComments = AutoMapper.Mapper.Map<MasterBService.Comment[], List<Comment>>(commentListFromMB);

                bs.RefreshCommentsTable(freshComments);

                List<Comment> commentList = bs.GetAllCommentsForSelectedAccomodation(objId);

                string[] messages = new string[] { "Successfully Get", "Comments successfully retrieved" };
                return Ok(commentList, HttpStatusCode.OK, "Successfully Get", messages);
            }
            else
            {
                return Error(HttpStatusCode.NotAcceptable, ModelState);
